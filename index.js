...
async function getRegistrationFee() {
    ...
    const price = level1 ? level1.price : 5.25;
    const salaryFund = level1 ? level1.salaryFund : 0;
    const fee = level1 ? level1.fee : 0;
...
async function distributeRegistrationCommissions(inviterId, newUserId) {
    ...
    if (!level1 || typeof level1.price !== 'number') {
        console.error("FATAL: Level 1 price not configured");
        return;
    }
    
    const commissionableAmount = level1.price;
    console.log(`📊 Distributing registration commission: ${commissionableAmount} ZTR`);
    
    // 1. Direct Commission (55%)
    await addCommission(inviterId, commissionableAmount * 0.55, 'Direct Commission', 'direct', 1, newUserId, 1);
    ...
    if (inviterWallet.exists()) {
        ...
        if (inviterData.exists() && inviterData.val().inviterId) {
            await addCommission(inviterData.val().inviterId, commissionableAmount * 0.07, 'Upline Commission', 'upline', 1, newUserId, 1);
        }
    }
    
    // 3. Team Commission (20%) - Split among existing direct members
    ...
    if (teamMembersSnapshot.exists()) {
        ...
        if (team.length > 0) {
            ...
            for (const member of team) {
                await addCommission(member.userId, sharePerMember, 'Team Commission', 'downline', 1, newUserId, 1);
            }
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        if (inviterWallet.exists()) {
            ...
            await inviterRef.child('airdropPoints').transaction(p => (p || 0) + 100);
...
app.get('/api/team/:userId', async (req, res) => {
    ...
    try {
        const teamMembers = [];
        const usersSnapshot = await db.ref('users').orderByChild('inviterId').equalTo(parsedUserId).once('value');
        ...
        res.json({ success: true, team: teamMembers, count: teamMembers.length });
